...
export default function Produtos() {
  ...
  return ( 
    <div>
      ...
      <div>
        <button className={style.buttonStyle} onClick={()=> setCounter(counter + 1)}>COUNTER - {counter}</button>
      </div>
      <div>
        <button className={style.buttonStyle} onClick={()=> setCounter2(counter2 + 1)}>COUNTER2 - {counter2}</button>
      </div>
...
<tfoot>
  <tr>
    <td colSpan={4}><div>
        <button className={style.buttonStyle} > <Link to={`/adicionar/produtos`}style={{ textDecoration: 'none' }}>Adicionar Produtos</Link></button>
      </div></td>
  </tr>
</tfoot>
      </table>
